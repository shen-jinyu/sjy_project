""""构建Gradio界面,然后输入问题,接收去请求并调用检索和生成逻辑"""

import gradio as gr
from file_to_chroma import check_collection_exists,create_and_fill_collection
# 上面是导入文件内的函数，分别是检查数据库是否已经存在,创建并存储新的数据库和路径

with gr.Blocks() as demo:                              # 创建一个自由布局的Gradio应用

    gr.Markdown("# 智能学习主教")                       # 添加标题

    with gr.Row():                                      # 创建水平分栏,内部组件将左右分布
        with gr.Column():                               # 在Row内创建左侧垂直列
            # 聊天窗口(聊天消息集合messages)
            ...
            # 发送消息函数
            def send_message(query,history):                        # 定义函数,模拟聊天机器人的消息处理流程,两个参数(用户输入,历史对话)
                
                history.append({"role":"user","content":query})     # 把用户输入追加到历史对话中
                # 调用llm对话
                resp = "AI接收到了:" + query                        # 构建一个简单的AI回复功能

                history.append({"role":"assistant","content":resp}) # 将AI回复也添加到历史对话中
                # 返回llm结果(chatbox)
                return history                                      # 返回历史对话
            # 发送消息处理
        ...
        with gr.Column():                           # 创建右侧垂直列
            ...
            # 数据库选项
            gr.Dropdown(choices=["aaa","bbb"],label="数据库名")  # 构建数据库选项,下拉选择
            # 刷新按钮
            gr.Button("刷新")                   # 构建刷新按钮
