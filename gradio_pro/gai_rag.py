...
def query_rag(query,history,collection_name=None):
    """通过用户query进行检索返回llm对话响应"""




    messages = [item for item in history]       # 创建空列表,用于存储完整的对话上下文
    ...
    # llm调用
    generator = llm_chat_with_history(       # 调用模型(返回思考内容:省略,返回输出值:需要)
        messages = messages,                # 消息对话传给参数
        model_name="glm-4-flash",           # 指定模型(4.7深度思考模型)
...
if __name__ == '__main__':
    ...
    # 使用stream模式，逐一提取模型返回token
    gen = query_rag(query,[],collection_name)
    ...
    for think,token in gen:
        ...
        if token:
            print(token,end="",flush=True)  # flush=True强制立即将输出内容从缓冲区写入终端（或输出流），而不是等待缓冲区满或程序结束。
    print()
